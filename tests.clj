...
(defn spelltest [tests]
  (let [results (flatten (for [[target wrongs] tests
			       wrong (str/split wrongs #" ")]
			   (cond
			    (= target (correct wrong)) :good
			    (*nwords* target) :bad
			    :else [:bad :unknown])))
	n (count (filter #{:good :bad} results))
	bad (count (filter #{:bad} results))
	unknown (count (filter #{:unknown} results))
	pct (* 100.0 (- 1 (/ bad n)))]
    {:bad bad :unknown unknown :pct pct :n n}))

(def tests1 {"access" "acess", "accessing" "accesing", "accommodation"
...
(def tests2 {"forbidden" "forbiden", "decisions" "deciscions descisions",
